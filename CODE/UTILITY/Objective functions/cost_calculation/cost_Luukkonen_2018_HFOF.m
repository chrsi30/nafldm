function[ cost ]= cost_Luukkonen_2018_HFOF( D, s_b, s_d , utility , prediction_flag)
...
idx = ismember(s_d.time , D.(tmp).HF_SAT_OF.LF.Time );
...
% LF
c1 = sum( ( (s_d.variablevalues( idx,end)' - D.(tmp).HF_SAT_OF.LF.Mean(idx_data) ).^2)./( D.(tmp).HF_SAT_OF.LF.SEM(idx_data).^2)) ;
