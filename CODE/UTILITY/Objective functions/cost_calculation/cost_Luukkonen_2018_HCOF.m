function[ cost ]= cost_Luukkonen_2018_HCOF( D, s_b, s_d , utility , prediction_flag)
...
idx = ismember(s_d.time , D.(tmp).HCOF.LF.Time );
...
c1 = sum( ( ( s_d.variablevalues( idx,end)' - D.(tmp).HCOF.LF.Mean(idx_data) ).^2)./( D.(tmp).HCOF.LF.SEM(idx_data).^2)) ;
...
c2 = sum( ( (  y2  -D.(tmp).HCOF.DNL.Mean(idx_data)' ).^2)./D.(tmp).HCOF.DNL.SEM(idx_data)' ) ;
...
% %%% Requirment that liver-fat should always increase 
% adhoc_inc = 0;
% if s_d.variablevalues( 1,end)*1.2 > s_d.variablevalues( end,end)
% adhoc_inc =  adhoc_inc + max(0, s_d.variablevalues( 1,end)*1.2  -  s_d.variablevalues( end,end)  ) +  36.4150;  
% end
% 
% adhoc = adhoc + adhoc_inc;
