function[ cost ]= cost_Johnston_2013_HC_OF_F( D, s_b, s_d , utility , prediction_flag)

tmp = 'Johnston2013';

if  prediction_flag == 0

idx = ismember(s_d.time , D.(tmp).HC_OF_F.LF.Time );
...
% LF
c1 = sum( ( ( s_d.variablevalues( idx,end)' - D.(tmp).HC_OF_F.LF.Mean(idx_data) ).^2)./( D.(tmp).HC_OF_F.LF.SEM(idx_data).^2)) ;
